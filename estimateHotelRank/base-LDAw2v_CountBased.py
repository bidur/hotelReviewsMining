...
##### Reference words of negative and positive opinion
...
'''
def estimateHotelRating_method_CountBased(currentHotelReviewsAsStr, currentHotel,  aspectDictByTopic,  AspectScoreNormalize=1):
	
	
	
	gAspectList = [ aspectDictByTopic[i] for i in range(0,len(aspectDictByTopic)) ]   # 2-d array of Aspects
	globalAspectList = [j for sub in gAspectList for j in sub] # flattern to 1-D list
	aspectsPresent = list( set(globalAspectList)  ) # aspects Present for this hotel
	
	aspectValuesDict = {}
	#initialize with aspects/nouns of the current hotel 
	aspectValuesDict = {key: 0. for key in aspectsPresent } # all items are set to 0 e.g: aspectValuesDict['room'] = 0
	...
	for eachLine in reviewSentList:
		...
		for word, wordFreq in wordCounter.iteritems():
			...
			if word in aspectsPresent:
				aspectValuesDict [ word ] += wordFreq 
	...
	print '                         estimateHotelRating_method_CountBased: ' +currentHotel
...
def processEachHotel(reviewsDF, minimumReviewCount,nGramNumber, minimumWordRepetition, aspectDictByTopic, opFileName, AspectScoreNormalizeFlag = 'YES' ):
	...
	for currentHotel in hotelNameList:
		...
		if AspectScoreNormalizeFlag == 'YES':
			...
			aspectValuesDict, currentHotelAspectNames = estimateHotelRating_method_CountBased(currentHotelDataAsStr, currentHotel,  aspectDictByTopic,  AspectScoreNormalize = hotelReviewCountDict[currentHotel])
		else:
			
			aspectValuesDict, currentHotelAspectNames = estimateHotelRating_method_CountBased( currentHotelDataAsStr, currentHotel, aspectDictByTopic, AspectScoreNormalize = 1  )
...
def main():
	...
	normalizeFlag = 'YES'  # totalAspectScore is deivide by totalReviews 
	topicMethod = 'w2v' #'LDA' or 'w2v'
	scoreMethod = 'Count Based' # 
	...
	if normalizeFlag == 'YES':
		...
		outputDir = 'base_'+topicMethod+'_CountBased_NORM_' + 'minReview-' + str(minimumReviewCount)+ '_wordProb-'+str(probThreshold) +'_'+ str(nGramNumber) +'-gram_'+ str(minimumWordRepetition) +'X/'
	else:
		outputDir = 'base_'+topicMethod+'_CountBased_minReview-' + str(minimumReviewCount)+ '_wordProb-'+str(probThreshold) +'_'+ str(nGramNumber) +'-gram_'+ str(minimumWordRepetition) +'X/'
