...
'''
def estimateHotelRating(currentHotel,  aspectDictByTopic,  hotel_NNJJ_Mat, nounList4Hotel, adjList4Hotel, AspectScoreNormalize=1):
	
	aspectValuesDict = {}
	#initialize with aspects/nouns of the current hotel 
	aspectValuesDict = {key: INTERCEPT for key in nounList4Hotel } # all items are set to 0 e.g: aspectValuesDict['room'] = 0
	...
	for aspect in aspectsPresent:
		#if aspect in nounList4Hotel:
			nounLoc = nounList4Hotel.index(aspect)
			# calculate aspect Weight in NN-JJ matrix
			for j in range( len(adjList4Hotel) ):
				occurrenceCount = hotel_NNJJ_Mat [nounLoc][j]
				if occurrenceCount > 0:
				
					currentAdjSentiment = float(TextBlob(adjList4Hotel[j]).sentiment.polarity) # polarity range [-1,1]
					aspectWeight = getAspectWeight4word(aspectDictByTopic, aspect)
					aspectScore  = (currentAdjSentiment *  occurrenceCount * aspectWeight )/ AspectScoreNormalize # aspect weight = Polarity * no. of occurrence of the noun & adj in hotel's Noun-ADJ matrix
					
					
					currentHotelAspectNames += ' . '+ aspect +'-' + adjList4Hotel[j] +':' + str(occurrenceCount )
						
					aspectValuesDict [ aspect ] += aspectScore # sum of all the aspectScore 
					
	aspectValuesDictSorted = collections.OrderedDict(sorted(aspectValuesDict.items()))
	print '                         estimateHotelRating: ' +currentHotel
	
	return aspectValuesDictSorted, currentHotelAspectNames # aspect and respective weights in a dict
...
def processEachHotel(df, minimumReviewCount,nGramNumber, minimumWordRepetition, aspectDictByTopic, opFileName, AspectScoreNormalizeFlag = 'YES' ):
	#General Hotel Informations
	hotelRatingDict , hotelReviewCountDict = getOverallHotelRating()
	
	allReviewList = df['review_text']
	allTitleList = df['review_title']
	allDataAsStr = ". ".join(map(str,allReviewList)) + ". ".join(map(str,allTitleList)) # optimize by sending the list here and handling the list in get_ngram()
	hotelNameList = sorted(set(df['name']))
	...
	for currentHotel in hotelNameList:
		...
		if (currentHotel in hotelReviewCountDict.keys() and hotelReviewCountDict[currentHotel] < minimumReviewCount):
			continue
		...
		allReviewList4Hotel = df.review_text[df.name == currentHotel] 
		allTitleList4Hotel = df.review_title[df.name == currentHotel] 
		allDataAsStr = ". ".join(map(str,allReviewList4Hotel)) + ". ".join(map(str,allTitleList4Hotel)) # optimize by sending the list here and handling the list in get_ngram()
	
		nGramListGlobal, nounList4Hotel, adjList4Hotel = get_ngram( allDataAsStr, nGramNumber, minimumWordRepetition) # nGramNumber=3 -> 3gram
		...
		if AspectScoreNormalizeFlag == 'YES':
			aspectValuesDict, currentHotelAspectNames = estimateHotelRating( currentHotel,  aspectDictByTopic,  hotel_NNJJ_Mat, nounList4Hotel, adjList4Hotel , AspectScoreNormalize = hotelReviewCountDict[currentHotel])
		else:
			aspectValuesDict, currentHotelAspectNames = estimateHotelRating( currentHotel, aspectDictByTopic,  hotel_NNJJ_Mat, nounList4Hotel, adjList4Hotel, AspectScoreNormalize = 1  )
		
		eachTopicValue4aHotel = INTERCEPT + sum(aspectValuesDict.values())
		...
		#howManyAspects = len( currentHotelAspectNames.split(' . ') ) - 1
		...
		hotelDataByTopicValue(currentHotel,hotelRating, aspectDictByTopic, aspectValuesDict, allHotelAspectFile)
...
def hotelDataByTopicValue(currentHotel,hotelRating, aspectDictByTopic, aspectValuesDict, allHotelAspectFile): # each Topic = each aspect
...
############ LDA incercept and coeffieients got from Linear Regression (case: Review Normalized ) , 3 gram case
INTERCEPT =  0.0031
COEFFICIENTS = [ 0.7 , -0.  ,  1.58,  2.  ,  0.49,  1.61,  0.77,  0.76,  2.18,
        1.06,  0.18,  4.58,  0.62,  0.6 ,  1.05,  0.48,  0.56,  0.83,
        0.51,  0.74]

'''
############ w2v incercept and coeffieients got from Linear Regression (case: Review Normalized ), 3 gram case
INTERCEPT =  0.0
COEFFICIENTS = [ 0.,  1.,  1.,  1.,  1.,  0.,  1.,  1., -0., -0., -0.,  0.,  0.,
        1.,  1., -0.,  1.,  1.,  0.,  1.]
...
def main():
	...
	normalizeFlag = 'YES'  # YES/NO . if YES -> totalAspectScore is deivide by totalReviews  # update COEFFICIENTS and INTERCEPT above
	...
	scoreMethod = 'sentiment' # cosine similarity
	...
	if normalizeFlag == 'YES':		
		...
		outputDir = topicMethod+'_SENTI_NORM_TEST_' + 'minReview-' + str(minimumReviewCount)+ '_wordProb'+topicMethod+'-'+str(probThreshold) +'_'+ str(nGramNumber) +'-gram_'+ str(minimumWordRepetition) +'X/'
	else:

		outputDir = topicMethod+'_SENTI_TEST_' + 'minReview-' + str(minimumReviewCount)+ '_wordProb'+topicMethod+'-'+str(probThreshold) +'_'+ str(nGramNumber) +'-gram_'+ str(minimumWordRepetition) +'X/'
	...
	# read the input file in pandas
	df = pd.read_excel(inputFile, sheet_name)
	'''
	AspectScoreNormalizeFlag = YES/NO
	'''
	resultDF = processEachHotel(df, minimumReviewCount,nGramNumber, minimumWordRepetition, aspectDictByTopic, opFileName,  AspectScoreNormalizeFlag = normalizeFlag)
